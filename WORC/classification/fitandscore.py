...
def fit_and_score(X, y, scoring,
    ...
    if labellength > 1 and not cc.is_multilabel_classifier(estimator):
...
def _fit_and_score(estimator, *args, **kwargs):
    if cc.is_survival_classifier(estimator) or (hasattr(estimator, 'estimator' and cc.is_survival_classifier(estimator.estimator))):
        return _survival_fit_and_score(estimator, *args, **kwargs)
    else:
        return _org_fit_and_score(estimator=estimator, *args, **kwargs)

def _survival_fit_and_score(estimator, feature_values, y, *args, **kwargs):
    y_restructured = Surv.from_arrays([row[0] for row in y], [row[1] for row in y])
