...
# Copyright 2024-2025 Biomedical Imaging Group Rotterdam, Department of
...
def plot_estimator_performance(prediction, label_data, label_type,
    ...
    output: string, default stats
        ...
        estimator will be returned. If scores, the scores will be returned. If
        shap, Shapley values will be returned.
    ...
    for i in iterobject:
        ...
        if output in ['decision', 'scores'] or crossval_type == 'LOO':
            ...
            # Use KernelExplainer as it is generic. Speed up with kmeans
...
def plot_shap_values(prediction, pinfo, ensemble_method='top_N',
                     ...
                     shap_output_file='shap_values.npz',
                     barplot_file='shap_barplot',
                     beeswarm_file='shap_beeswarm',
                     waterfall_file='shap_waterfall'):
    ...
    if isinstance(pinfo, list):
        pinfo = ''.join(pinfo)
    if isinstance(ensemble_method, list):
        ensemble_method = ''.join(ensemble_method)
    if isinstance(ensemble_size, list):
    ...
    shap_values, X_test, feature_labels = plot_estimator_performance(
        prediction, pinfo, [label_type],
        alpha=0.95,
    ...
    print(f"SHAP values shape: {shap_values.shape}")  # shape: (n_splits, n_samples, n_features)

    # Compute mean and std SHAP values across folds (abs for importance)
    abs_shap = np.abs(shap_values)  # shape: (folds, samples, features, classes)
    mean_shap = np.mean(abs_shap[..., 1], axis=(0, 1))  # select class 1
    std_shap = np.std(abs_shap[..., 1], axis=(0, 1))
    ...
    np.savez(shap_output_file, shap_values=shap_values, features=feature_labels, X_test=X_test)

    # Plot bar plot: mean ± std
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(range(len(sorted_mean)), sorted_mean, yerr=sorted_std, capsize=4)
    ax.set_xticks(range(len(sorted_features)))
    ax.set_xticklabels(sorted_features, rotation=90)
    ax.set_ylabel('Mean |SHAP value|')
    ax.set_title('Feature Importance (SHAP)')
    fig.tight_layout()
    plt.savefig(f"{barplot_file}.png", dpi=300)
    matplot2tikz.save(f"{barplot_file}.tikz")

    plt.close(fig)

    # Plot beeswarm: one fold to illustrate
    ...
    plt.tight_layout()
    plt.savefig(f"{beeswarm_file}.png", dpi=300)
    matplot2tikz.save(f"{beeswarm_file}.tikz")
    plt.close(fig)

    # Plot waterfall for one representative sample
    sample_idx = 0
    explainer = shap.Explanation(
    ...
    fig = plt.figure()
    shap.plots.waterfall(explainer, show=False, max_display=15)
    plt.tight_layout()
    plt.savefig(f"{waterfall_file}.png", dpi=300)
    matplot2tikz.save(f"{waterfall_file}.tikz")
    plt.close(fig)

    print(f"Saved SHAP values to {shap_output_file}")
    print(f"Saved barplot to {barplot_file}.png and .tikz")
    print(f"Saved beeswarm to {beeswarm_file}.png and .tikz")
    print(f"Saved waterfall to {waterfall_file}.png and .tikz")
...
if __name__ == '__main__':
    # Test
    prediction = r"C:\\Users\\795023\WORC\\output\\WORC_Example_STWStrategyHN_py311_250523_run4\\estimator_all_0.hdf5"
